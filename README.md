...
1. **HTTP 分发**：libevent 回调如何路由到四个业务 handler
2. **分片断点续传**：分片文件如何临时存储、如何合并、如何处理网络中断重传
3. **原子写入**：为什么用临时文件 + `rename`，避免什么问题
4. **MySQL 一致性**：主键 upsert + 事务保证元数据与磁盘文件一致
5. **安全校验**：路径穿越防护、上传大小限制、文件名合法性校验
6. **异步日志**：双缓冲 + 后台刷盘如何降低业务线程的 IO 阻塞
7. **日志轮转**：按大小/日期滚动，按数量/时长清理
8. **生命周期管理**：用构造函数注入替代全局指针，明确对象所有权
