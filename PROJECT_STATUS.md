...
已完成 P1 的元数据存储抽象，为后续扩展不同元数据后端做铺垫：
...
当前项目已经进入收尾阶段，主线能力已经足够用于学习展示和面试讲解。

已完成的主线能力：

- HTTP 文件上传、列表展示、下载、删除。
- 普通存储和深度压缩存储。
- 深度存储避免对常见已压缩格式二次压缩。
- 下载支持 Range 断点续传。
- 文件名、URL、HTML 输出等基础安全校验。
- 文件写入和元数据写入采用临时文件加原子替换。
- 元数据字段扩展：存储类型、压缩状态、原始大小、落盘大小、上传时间、Content-Type、内容 hash。
- 元数据存储抽象为 `MetadataStore`，当前有 `JsonMetadataStore` 和 `MysqlMetadataStore` 两种实现。
- 默认配置使用 JSON 元数据，方便项目开箱运行。
- MySQL 后端已经完成真实编译和上传、下载、删除验证。
- MySQL 密码通过环境变量读取，不写入配置文件。
- MySQL 错误日志已增强，排错时会记录连接上下文和操作信息，但不记录密码。
- Web 文件列表已支持服务端分页，默认每页 10 条，最大 `page_size` 限制为 100。
- 异步日志系统支持后台刷盘、日志轮转和基础清理。
- 服务端核心资源已做 RAII 管理，包括 libevent 资源、下载文件 fd、元数据读写锁。
- 历史测试数据已清空，不做 JSON 到 MySQL 的历史迁移工具。

当前策略：

- 不再继续追加大功能，避免项目复杂度超过面试可讲清楚的范围。
- 默认保持 `metadata_store=json`，MySQL 作为可配置增强能力展示。
- 后续重点从“继续开发”切换到“整理文档、压缩讲解主线、准备面试问答”。
...
- 准备一套 3 到 5 分钟项目讲解主线：
  - 项目定位：C++ + libevent 的轻量云存储服务，集成自研异步日志系统。
  - 核心链路：上传、元数据记录、列表、下载、删除。
  - 可靠性：临时文件原子替换、Range 下载、深度存储压缩状态记录。
  - 工程化：RAII、读写锁、模块拆分、配置化。
  - 扩展性：`MetadataStore` 抽象，JSON/MySQL 双后端。

- 准备常见追问答案：
  - 为什么默认 JSON，而不是默认 MySQL？
  - 为什么 MySQL 只存元数据，不存文件内容？
  - Range 下载怎么解析？
  - 深度存储怎么判断是否需要解压？
  - 异步日志如何避免阻塞业务线程？
  - RAII 在项目里解决了什么问题？
...
- 更新 `README.md`，把运行方式、MySQL 可选配置、核心功能和面试讲解点写清楚。
- 做一次最终 smoke test：
  - `make`
  - 访问列表页
  - 上传普通文件
  ...
  - 删除普通文件
  - Range 下载
  - 可选：切换 MySQL 后端再验证一次上传/查表/删除
- 清理运行时测试文件：

```bash
rm -f storage.data
rm -rf low_storage deep_storage logfile
mysql -u cloud_user -p cloud_storage -e "TRUNCATE TABLE file_metadata;"
```

### 未来优化方向

这些方向先不作为当前阶段目标，避免项目继续膨胀。面试时可以作为“后续演进”来讲。

存储和元数据：

- 基于文件 hash 做重复内容检测，为后续秒传/去重打基础。
- 将当前 `fnv1a64` 内容指纹升级为 SHA-256，用于更强完整性校验。
- 增加后台一致性扫描，检查元数据和磁盘文件是否匹配。
- JSON 元数据损坏时自动备份为 `storage.data.bad.<timestamp>`，并以空元数据继续启动。
- 如果后续确实需要保留历史数据，再补 JSON 到 MySQL 的迁移工具。

查询和页面：

- 将搜索、排序也下推到后端，尤其是 MySQL 后端可以用 SQL 分页查询。
- 文件列表增加按存储类型过滤。
- 增加更细的文件元数据展示页。

系统能力：

- 增加用户体系和鉴权，但当前阶段不建议优先做。
- 梳理并优化已有线程池，将线程池用于后台维护任务。
- 增加对象存储后端，例如本地磁盘之外的存储服务。
